...
import leadLastStatusService from "../../Services/leadLastStatus";
...
const EditLead = () => {
  ...
  const fetchLead = async (uuid) => {
    try {
      const response = await leadsService.getByUuid(uuid);
      // console.log(response.data)
      setLead(response.data); // Store the fetched data
      fetchLastStatusData(response.data?.id);
  ...
  useEffect(() => {
    if (lead) {
      form.setFieldsValue({
        ...
        package: lead.package_id ? String(lead.package_id) : undefined,
        gym: lead.gym_id ? String(lead.gym_id) : undefined,
        dob: lead.dob ? moment(lead.dob) : null,
        goal_ids: lead.goal_ids ? JSON.parse(lead.goal_ids) : [],
      });
    }
  }, [lead]);
  ...
  const fetchLastStatusData = (id) => {
    leadLastStatusService
      .getByLeadId(id)
      .then((res) => {
        // console.log(res.data);
        setLastStatusData(res?.data);
      })
      .catch((error) => {
        console.log(error);
      });
    ...
    <Container fluid className="email-wrap bookmark-wrap todo-wrap">
        ...
        <Form form={form} layout="vertical" onFinish={onFinish}>
          <Row gutter={16}>
            ...
            {step === 2 && (
              <>
                <Col xs={24} sm={12} md={8} lg={6}>
                  <Form.Item label="Goals" required name="goal_ids" rules={[{ required: true, message: "Goals is required" }]}>
                    <Select
                      mode="multiple"
                      allowClear
                      value={goalsValue}
      ...
      <Col sm="12" className="my-2">
          <Card>
              ...
              <div className="card-block row mt-2">
                          ...
                          <Table className='table-light'>
                              <thead>
                                  <tr>
                                      ...
                                      <th scope="col">{'Custom Notes'}</th>
                                      <th scope="col">{'Date'}</th>
                                  </tr>
                              </thead>
                              <tbody>
                                  {
                                      lastStatusData.map((item) => {
                                          return (
                                              <tr key={item.id}>
                                                  <th scope='row'>{item.id}</th>
                                                  <td className={`${item.status === 'Pending' ? 'text-danger' : 'text-success'}`}>{item.follow_up_status}</td>
                                                  <td>{item.custom_notes}</td>
