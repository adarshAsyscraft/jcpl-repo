...
import { H5, H6, P, UL, LI } from "../../AbstractElements";
import SvgIcon from "../Common/Component/SvgIcon";

const UpcomingSchedule = ({ data, pagination }) => {
  ...
  const handlePageChange = (page) => {
    if (page >= 1 && page <= totalPages) {
      setCurrentPage(page);
      pagination({ current: page, pageSize: itemsPerPage });
    }
  };
    ...
    <Card className="schedule-card">
      <CardHeader className="card-no-border">
        <div className="header-top">
          <H5 attrH5={{ className: "m-0" }}>Upcoming Schedule</H5>
        </div>
      </CardHeader>
      <CardBody className="pt-0">
          ...
          {paginatedData.map((item, i) => (
            <LI key={i} attrLI={{ className: `primary` }}>
              <div>
                ...
                <P className="mb-1">{item.user_name} ({item.user_email})</P>
                <UL>
                  <LI attrLI={{ className: "f-light" }}>
                    <SvgIcon iconId={'bag'} className="fill-icon f-light" />
                    <span>{new Date(item.booking_date).toLocaleDateString('en-CA')}</span>
                  </LI>
                  <LI attrLI={{ className: "f-light" }}>
        ...
        {/* Pagination */}
        <Pagination aria-label="Page navigation example" className="pagination-primary d-flex justify-content-center mt-2">
