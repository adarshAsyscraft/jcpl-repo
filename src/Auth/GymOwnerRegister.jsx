...
import { EyeOutlined, EyeInvisibleOutlined ,CheckCircleTwoTone ,CloseCircleTwoTone} from "@ant-design/icons";
...
const GymOwnerRegister = () => {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      if(error.response?.data?.message === "OTP not found or not requested."){
  ...
  const SendOTP = () => {
    ...
    axios
      .post(`${API_URL}/users/send-otp`, { identifier: email })
      .then((res) => {
  ...
  const VerifyOTP = () => {
    console.log("hit");
    if (!email||!otp) {
      toast.error("Please Enter a valid email OR OTP address");
      return false;
    }
    axios
      .post(`${API_URL}/users/verify-otp`, {email,otp })
      .then((res) => {
        console.log(res.data);
        if (res.data.success) {
          setIsOTPVerify(true)
                     ...
                     {getOTP && <Col xs={12}>
                        <FormGroup>
                          <Flex gap="middle" align="flex-start" vertical>
                            <Label>OTP {" "} <Tag color="green">{getOTP}</Tag> </Label>
                            <Input.OTP
                              length={6}
                              type="number"
                              onChange={(e) => setOtp(e)}
                              ...
                              />
                             {isOTPVerify? <Label style={{color:"#52c41a"}}>  <CheckCircleTwoTone twoToneColor="#52c41a" />{" "}Verified</Label>
                              :<Label style={{color:"rgb(255 35 35)"}}>  <CloseCircleTwoTone twoToneColor="rgb(255 35 35)" />{" "} Unverified</Label>}
                          </Flex>
                        </FormGroup>
                      </Col>}
                      <Col xs={12}>
